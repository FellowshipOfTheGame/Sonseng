    ...
    SignInWithGoogle: function (objectName, callback, fallback) {
        ...
        try {
            ...
            firebase.auth().signInWithPopup(provider).then(function (result) {
                result.user.getIdToken(false).then(function (res){
                    unityInstance.Module.SendMessage(parsedObjectName, parsedCallback, JSON.stringify({uid: result.user.uid, token:res}));
